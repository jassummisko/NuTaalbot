...
class channelManagerCog(commands.Cog):
    ...
    async def beantwoord(self, i9n: discord.Interaction):
        assert hasattr(i9n.channel, "parent"), botResponses.NOT_IN_FORUM_ERROR
    ...
    async def debug_gettagids(self, ctx: commands.Context):
        assert genUtils.isStaff(ctx.message.author), botResponses.NOT_STAFF_ERROR
        assert isinstance(ctx.channel.parent, ForumChannel), botResponses.NOT_IN_FORUM_ERROR
